...
# Configure coding agent tools (Claude Code, Codex, Gemini CLI)
[group('tools')]
ai-coding-agents: qmd
    ...
    {
      echo '<!-- Generated by: just ai-coding-agents -->'
      ...
      echo
      # Append output style (strip YAML frontmatter, demote headings)
      awk '
        BEGIN { fm = 0 }
        /^---$/ { fm++; next }
        fm >= 2 { if (/^#/) print "#" $0; else print }
      ' claude/output-styles/personal.md
    } | awk '/^$/{if(b)next;b=1;print;next}{b=0;print}' | tee codex/AGENTS.md > gemini/GEMINI.md
    ...
    for cmd_md in claude/commands/*.md; do
      cmd_name=$(basename "${cmd_md}" .md)
      dest="$HOME/.gemini/commands/${cmd_name}.toml"
      echo "  ${cmd_name}"
      desc=$(yq --front-matter=extract '.description' "${cmd_md}" \
        | sed 's/"/\\"/g')
      body=$(awk 'BEGIN{fm=0} /^---$/{fm++;next} fm>=2{print}' "${cmd_md}" \
        | sed 's/\$ARGUMENTS/{{{{args}}/g' \
        | sed 's/\.claude\//\.gemini\//g')
      {
