...
flowchart TD
    ...
    subgraph qmd["qmd (BM25 + Vector + Reranking)"]
        ...
        c4["homunculus-conversations"]
    end

    observe -->|append| obs
    stop -->|increment session_count| identity
    stop -->|generate summary| sessions
    stop -->|summarise conversation| conversations
    ...
    promote -->|create file| convictions["convictions/*.md"]
...
A session is a structured activity log generated by the stop
hook when a Claude Code session ends. It captures _what tools
were used_ and _which files were touched_, not dialogue
content. Sessions form a chronological record linked together
via story threading.
...
- **Metadata, not dialogue**: Captures tool stats and files
  touched. For what was _discussed_, see
  [conversations](#whats-a-conversation)
...
A conversation is an LLM-generated digest of a Claude Code
session's dialogue. While [sessions](#whats-a-session) capture
_what tools were used and which files were touched_,
conversations capture _what was discussed and decided_. Both
share the same `session_id` for cross-referencing.
...
# Conversation: my-project

## Goals

Refactor configuration loading to support multiple environments.

## Decisions

Chose TOML over YAML for config format due to better type
support. Kept backwards compatibility with existing JSON configs.

## Outcomes

Implemented dual-format config loader with environment
detection. Tests passing. TODO: migration script for existing
deployments.
```

**Properties:**

- **LLM-summarised**: Raw dialogue is extracted from the
  session JSONL and summarised by haiku into a structured
  format (Goals, Decisions, Outcomes)
- **Linked to sessions**: Shares `session_id` with the
  corresponding session summary. Sessions track tools and
  files, conversations track reasoning and decisions.
- **Searchable**: Indexed in the `homunculus-conversations`
  qmd collection for recall
- **Backgrounded**: Summarisation runs asynchronously after
  session close so it never blocks
...
4. Summarises the session conversation in the background:
   extracts human/assistant dialogue from the session JSONL,
   sends to haiku for structured summarisation, saves to
   `conversations/` with matching `session_id`
5. Triggers the observer agent in the background to process
   accumulated observations (sets `HOMUNCULUS_OBSERVER=1` to
   prevent the observer from observing itself)
6. After a successful observer run, auto-promotes qualifying
...
completes, queries qmd for past sessions, episodes, and
conversations involving the same file. Runs asynchronously so
it adds minimal latency. Context arrives as a `[RECALL]`
system reminder, informing subsequent decisions.

Three qmd searches (sessions, episodes, conversations) run in
parallel to minimise wall-clock time. Requires qmd to be
installed; silently skips if unavailable.
...
Mature instincts graduate to individual conviction files in
`convictions/` automatically after each observer pass.
Convictions are injected at session start (before instincts).
They don't decay independently, but are auto-revoked (file
deleted) if the underlying instinct's confidence drops below
0.40.
...
An individual file in `~/.claude/homunculus/convictions/`,
using the same filename as the source instinct:

```markdown
---
name: launchctl-operations
action: "Use `sleep 5` between `launchctl unload` and `launchctl load`. Validate plist with `plutil -lint` before loading."
domain: tooling
promoted_at: 2026-02-14T10:00:00Z
source_instinct: tooling-launchctl-operations.md
---
```
...
- If a promoted instinct decays below 0.40, the stop hook
  deletes the conviction file and unsets `promoted: true`
  so the instinct can be re-promoted if the pattern
  resurfaces
...
Four collections bridge them:

| Collection                  | Contents                          | Used by                                  |
| --------------------------- | --------------------------------- | ---------------------------------------- |
| `homunculus-conversations`  | Dialogue digests (goals, decisions, outcomes) | Recall hook                              |
...
| `homunculus-sessions`       | Activity logs (tools used, files touched)      | Stop hook (story threading), Recall hook |
...
```text
~/.claude/homunculus/
├── conversations/              # LLM-summarised session dialogues (*.md)
├── convictions/                # Promoted instincts (*.md, auto-revoked if stale)
...
| `/homunculus-status`   | Show system health and instinct confidence      |
...
- Observations and conversation summaries stay **local** on
  your machine
- Only **instincts** (patterns) can be exported for sharing
- Conversation summaries contain dialogue digests, not raw
  code or full transcripts
