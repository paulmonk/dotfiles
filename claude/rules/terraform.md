...
- Use `checkov` or `tfsec` for linting code for security compliance
- Use `terraform-docs` to generate documentation

## Module and Repository Structure

Organise your Terraform modules and repositories as follows:
...
- `providers.tf`: Terraform version, Provider configurations and requirements.
- `main.tf`: Add Backend configuration for state storage here (only needed for root modules)
- `locals.tf`: Local value definitions.
- `modules/`: Local module definitions in `./modules/<module_name>`.
...
- Use `terraform validate` to check syntax and internal consistency
- Use TFLint for additional static code analysis and organisation-specific rules
