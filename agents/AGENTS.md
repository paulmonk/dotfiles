...
> **Claude Code**: Skip this section. These rules have `globs` frontmatter and are auto-loaded when you touch matching files.

For other agents, read the relevant guide before writing code in that language:

- [Rust](rules/rust.md) - `*.rs`, `Cargo.toml`
- [TypeScript](rules/typescript.md) - `*.ts`, `*.tsx`
- [Python](rules/python.md) - `*.py`, `pyproject.toml`
- [Go](rules/go.md) - `*.go`, `go.mod`
